...
The prototype verifier can correctly checking contract with primitve data type such as \texttt{Int} or \texttt{Bool}, but still we may improve the it in several aspects:
\begin{itemize}
...
\item Supporting dependent contract which means the post-condition can be dependent on some value from pre-condition. In order to do that, we cloud use a \texttt{lambda} which takes an arugment to wrap post-condition.
