...
\item the state includes a \textit{time} component, which encodes the execution contexts
\end{itemize}

Figure \ref{fig:abstractcesk} shows the state space of abstracting CESK* machine. Notation: we use hat to denote it's an abstract component, for example $\widehat{Env}$ is abstract environment.
...
\sigma \in \widehat{Store} & = \widehat{Addr} \rightarrow \mathcal{P}(D) \\
\kappa \in Cont & = \texttt{Halt} ~|~ \texttt{LetK}(v,e,\rho,\kappa) \\
