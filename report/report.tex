...
As we mentioned before, the key idea is take contracts as abstract values. In this part, we will describe the computation on these contracts and how to check these contracts. Contracts depict the boundaries of a value, and in our project, we define contract as \textit{a type tag with a set of predicates on value}.
...
\begin{itemize}
\item pre-contract, post contract ...
...
Since a contract is type tag with a set of predicates, and we can obtain program's runtime behavior(i.e. the abstract value) from the abstract interpreter, so the contract checking are actually check is the runtime behavior a subset of user-defined contract. To achieve this, we first check is the type tag are same, if not then reject it directly; otherwise, we check is the predicates of abstract value a subset of contract's predicates.
