...
\begin{itemize}
\item Verify the user provided contracts and user defined functions are conpatible. e.g.: assume a given contract is\texttt{(:\ id\ (->\ Int\ Bool))} and a given function is \texttt{(lambda\ id\ \(x\)\ x)}, this function will be rejected.

\item Verify the use of the defined functions are compatible with the contracts. e.g.: assume a given function $id$, its definition is the same as that in the item above. For a dynamic typing language without any limitation, from this definition, we can say $id$ is polymorphism with input and output have the same type (and value). Let's apply it:\\
\begin{figure}[h!]
\lstset{language=Lisp}
\begin{lstlisting}[frame=single]
...
Consider two cases: 1) The given contract is \texttt{(:\ id\ (->\ Int\ Int))}. Since the above code also applies $id$ on \texttt{Bool}, it will be rejected. 2) The given contracts are \texttt{(:\ id\ (->\ Int\ Int))} and \texttt{(:\ id\ (->\ Bool\ Bool))}. The code will be accepted. 
\end{itemize}
