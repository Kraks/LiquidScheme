...
Unlike the concrete CESK machine, the transition function in abstract CESK machine is from a state to a set of states:
...
where 
\begin{align*}
clo((\lambda~(v)~body), \rho') & \in \mathcal{A}(aexp_0, \rho, \sigma) \\
\rho'' & = \rho'[v \rightarrow a] \\
